...
| Build fails at `pip3 install` with `externally-managed-environment` | Vercel → the deployment → Build Logs | Vercel's build image manages Python via `uv` (PEP 668) and refuses a bare `pip3 install` into the system environment. The build container is thrown away after every build, so there's nothing to protect - pass `--break-system-packages` (already in this repo's `vercel.json`). |
